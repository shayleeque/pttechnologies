...
class ERRPAGE:
    TRIGGER_MAP = {
        ...
        "400 Invalid Protocol": {"method": "raw_malformed", "protocol": "FOO"},
        "505 Invalid HTTP Version": {"method": "raw_malformed", "version": "9.8"}
    }
    ...
    def _get_response(self, trigger_config: Any) -> Optional[object]:
        ...
        if isinstance(trigger_config, str):
            ...
            if trigger_config.get("method") == "raw_malformed":
                if "protocol" in trigger_config:
                    return self.helpers.send_raw_malformed(
                        self.base_url, 
                        protocol=trigger_config["protocol"], 
                        timeout=3
                    )
                elif "version" in trigger_config:
                    return self.helpers.send_raw_malformed(
                        self.base_url, 
                        version=trigger_config["version"],
                        timeout=3
                    )
            else:
    ...
    def _analyze_error_content(self, content: str, trigger_name: str) -> List[Dict[str, Any]]:
        """
        Analyze error page content against known patterns.
        
        Args:
            content: Error page content to analyze
    ...
    def _match_pattern(self, content: str, pattern_def: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        flags_str = pattern_def.get('flags', 'i')
        re_flags = 0
        ...
        try:
            ...
            if self.args.verbose:
                ptprint(f"Invalid regex pattern: {e}", "INFO", not self.args.json, indent=8)
            return None
